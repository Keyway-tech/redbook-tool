...
本 skill **不自带依赖副本**：所有依赖 skill 安装/查找于「当前 agent 的 skills 根目录」，该目录由 `ensure_deps.sh` 在**每次任务开始时实时解析**（优先 `$AGENT_SKILLS_DIR`，否则取本 skill 父目录），**不写死**到固定路径，避免跨运行时死依赖。各依赖的实际位置恒为 `<skills_root>/<name>/SKILL.md`，每次重新判定；`ensure_deps.sh` 负责检查 / 安装 / 更新（**不向本仓库写入任何状态文件**）。主 skill 调用任一依赖时，按名称到运行时解析出的 `<skills_root>/<name>/` 读取其 SKILL.md 并完整遵循其规范流程，不裁剪其决策逻辑。主 skill 仅负责流程控制、信息传递、主线一致性自检与最终结果输出；本文件规定「传什么、收什么、哪些点主 skill 不得插手」。所有相对路径均以各依赖自身目录 `<skills_root>/<name>/` 为基准。

依赖总表（9 个，与 SKILL.md「环境检测」、`ensure_deps.sh` 三处一致）：

| 依赖 | 用途（流程步骤） | 缺失时安装来源 |
|---|---|---|
| xhs-real-keywords | 热搜词采集（步骤 3） | `lsuyu899-tech/xhs-real-keywords` 子路径 `skills/xhs-real-keywords` |
| dbs-content | 选题 + 行文方向 + 主标题草案（步骤 4） | `dontbesilent2025/dbskill` 对应子路径 |
| dbs-hook | 钩子生成（步骤 4） | `dontbesilent2025/dbskill` 对应子路径 |
| dbs-xhs-title | 主标题定稿校验（步骤 4） | `dontbesilent2025/dbskill` 对应子路径 |
| multi-search-engine | 实证数据检索（步骤 5） | 本机已装，无自动安装来源 |
| content-deai-engine | 写稿与逐图文案（步骤 6、8） | `lanyasheng/content-deai-engine` |
| dbs-resonate | 共鸣审稿（步骤 7、8） | `dontbesilent2025/dbskill` 对应子路径 |
| no-ai-slop | 去 AI 味（步骤 7） | `petergyang/no-ai-slop` 子路径 `skills/no-ai-slop` |
| guizang-social-card-skill | 图文卡出图（步骤 10） | `op7418/guizang-social-card-skill` |

---

## 1. xhs-real-keywords（热搜词采集，步骤 3）

- **路径**：`<skills_root>/xhs-real-keywords/SKILL.md`
...
- **后续动作（主 skill 职责）**：按与选题思路的相关度筛选 **5–10 个最贴近的搜索关键词**写入信息包，并保留各自来源证据链接（供步骤 4 标题与步骤 6 正文自然融入、供用户核验）；不足 5 个时调整种子词再采一轮，仍不足交用户裁决（显式放行或继续重采）。
...
- **路径**：`<skills_root>/dbs-content/SKILL.md`
...
- **后续动作（主 skill 职责）**：一致性自检（选题须源自用户思路与热搜词）后，与 dbs-hook 钩子、dbs-xhs-title 主标题定稿一起在步骤 4 一次性呈请用户确认（编号选项一站通过 / 调整）。
...
- **路径**：`<skills_root>/dbs-hook/SKILL.md`
...
- **完整执行**：读取其 SKILL.md，遵循其阶段流程（接收文案 → 内容质量诊断 → 生成优化方案：素材提取 / 素材增补 / 悬念制造三方法共 10–15 条 → 内部消化选定 Top1）。**若其 Phase 2 判定素材不足而停止优化，主 skill 把停止结论原样转达用户并回步骤 2 补充素材，禁止强迫生成、禁止主 skill 代写钩子。**
- **主 skill 接收**：钩子候选列表（含各自公式结构说明）+ Top3 推荐。
- **后续动作（主 skill 职责）**：筛选与图文载体适配的钩子（去除仅适用于视频口播的表述），与 dbs-content 选题/方向、dbs-xhs-title 标题定稿一起在步骤 4 一次性呈请用户确认；未选定禁止进入步骤 5。
- **禁止**：主 skill 跳过该依赖自写钩子、替用户选定钩子、把视频专用表述原样塞进图文。

---

## 3.1 dbs-xhs-title（主标题定稿校验，步骤 4）

- **路径**：`<skills_root>/dbs-xhs-title/SKILL.md`
- **职责边界**：小红书标题公式专家——从验证过的爆款标题公式中挑对的、用对的、说明为什么用这个。它只产标题（定稿校验），不定选题、不写正文。
- **传入**：dbs-content 在步骤 4 产出的主标题草案 + 选题/方向/钩子上下文。
- **完整执行**：读取其 SKILL.md，遵循其标题公式流程产出定稿结论。主 skill 不干预其公式选择与排序。
- **主 skill 接收**：标题定稿值；作为固定值供步骤 6 封面标题槽使用。
- **禁止**：主 skill 自行拟定或改写标题、替用户选定标题。

---

## 4. multi-search-engine（实证数据检索，步骤 5）

- **路径**：`<skills_root>/multi-search-engine/SKILL.md`
...
- **后续动作（主 skill 职责）**：将检索报告并入步骤 6 写稿信息包；真实数据在终稿中须标注来源；检索不到的信息标注「待核实」。
- **禁止**：主 skill 编造检索结果、把未经验证的来源当事实写入终稿、把检索噪声直接塞入正文。

---

## 5. content-deai-engine（写稿与逐图文案，步骤 6、8）

- **路径**：`<skills_root>/content-deai-engine/SKILL.md`（含 `references/anti-ai-patterns.md`、`platform-templates.md`、`preflight-checklist.md`）
...
- **传入（初稿，步骤 6）**：完整信息包——选题思路 + 人群画像 + 图文贴目的 + 热搜词（5–10 个）+ 已确认选题 + 已确认行文方向 + 已确认钩子 + 步骤 4 确认标题（固定值，封面标题槽）+ 实证检索报告（带来源）+ 用户配图（如有）+ 目标平台「小红书」+ 素材来源标注。明确要求：按小红书平台模板（痛点场景 → 亲历细节 → 3 步做法 → 互动提问）产出；热搜词自然融入正文 ≥2–3 个；数据对应检索报告来源；无法验证的信息标注「待核实」。
- **完整执行**：读取其 SKILL.md，遵循其 0) 先决条件 → 1) AI 味诊断 → 2) 四步重写 → 3) 平台适配 → 4) 标准输出 → 5) 质量门禁 → 6) 三角色自检 全流程。不得裁剪其重写与自检逻辑。
- **写稿记忆隔离（强制）**：调用 content-deai-engine 写稿或修订时，主 skill 仅向其传入本轮输入（步骤 4 确认的选题/方向/钩子/标题、步骤 5 检索报告、本轮 dbs-resonate 或 no-ai-slop 给出的修改方向、用户最新修改方向），**禁止传入也不允许其引用任何前几轮的写稿产物**（草稿 / 旧稿 / 旧逐图文案 / 历史版本任何字段）；每轮必须以本轮输入为唯一上下文重写，不沿用前轮措辞、结构、句式。
- **主 skill 接收**：标准输出——正文 / 评论区首评 / 标签建议 / 发布前风险提示 + 「逐图文案」块。封面标题由步骤 4 确认值固定提供（本步骤封面标题槽须等于该值），随步骤 8 由用户确认。
- **传入（修订，步骤 7、8）**：dbs-resonate 报告中的【具体改法】与 no-ai-slop 的去 AI 味稿，原样作为修改方向（或并入用户修改方向），按其修订模式（四步重写 / 快速调用模式）落实一轮并吸收去味修改。
- **后续动作（主 skill 职责）**：初稿进入步骤 7 审稿循环（dbs-resonate 共鸣 + no-ai-slop 去 AI 味）；修订稿回 dbs-resonate 复审 + no-ai-slop 去味；用户确认（步骤 8）后进入出图。
- **禁止**：主 skill 自行写稿 / 改稿、写稿 skill 不按修改方向修改、臆造细节或来源、终稿不含与选题强相关的热搜词。

---

## 5.1 no-ai-slop（去 AI 味，步骤 7）

- **路径**：`<skills_root>/no-ai-slop/SKILL.md`
- **职责边界**：去 AI 味重写专家——按其行为准则重写文本，去除 AI 生成痕迹，保留作者原意与口语感。**它是去 AI 味专责者，不替代 dbs-resonate 的共鸣诊断，也不替代 content-deai-engine 的写稿。**
- **传入**：步骤 7 审稿循环中 content-deai-engine 修订后的当前稿。
...
## 6. dbs-resonate（共鸣审稿，步骤 7、8）

- **路径**：`<skills_root>/dbs-resonate/SKILL.md`
...
- **放行标准（主 skill 控制，与 no-ai-slop 并联）**：五维全部有效、无删除/强化建议（共鸣达标）**且** no-ai-slop 判定无 AI 味（去味达标）→ 放行，进入步骤 8；否则【具体改法】即为修改方向，原样转交 content-deai-engine 修订（并交 no-ai-slop 去味）后复审。**停滞即停（强制）**：主 skill 对比本轮 content-deai-engine 产出与上一轮的「问题点 / 修改方向」——若本轮改动所针对的问题与上一轮完全一致（即 AI 改来改去都在处理同一个点而未推进），主 skill **立即停止审稿循环**，向用户报告阻塞点（双方结论 + 上一轮问题点），由用户裁决（指定新方向 / 接受现状 / 终止任务）。**禁止以「再试一轮」的方式继续内耗。**
- **后续动作（主 skill 职责）**：向用户展示诊断结论与修订说明（步骤 7 确认时）。
- **禁止**：主 skill 自行按建议改稿、跳过审稿直接请用户确认、把诊断报告当作终稿。

---

## 7. guizang-social-card-skill（图文卡出图，步骤 10）

- **路径**：`<skills_root>/guizang-social-card-skill/SKILL.md`
...
> 本模型确保：每个依赖 skill 不仅自身存在，其**运行时深度依赖**也齐备；且 git 类依赖**始终与上游保持最新**。全部在任务开始前的环境检测中由 `ensure_deps.sh` 强制完成，每次运行都做。

### 1. 深度依赖映射（在 `ensure_deps.sh` 的 `deep_deps_of()` 维护）

| 依赖 skill | 深度依赖键 | 含义 | 校验点 |
|---|---|---|---|
| `guizang-social-card-skill` | `playwright` | Playwright 模块 + Chromium 二进制 | `<skills_root>/guizang-social-card-skill/node_modules/playwright` + `chromium.executablePath()` |
| `xhs-real-keywords` | `playwright` | 浏览器自动化采集小红书联想热词 / 大家都在搜 | `<skills_root>/xhs-real-keywords/node_modules/playwright`（与 guizang 共用同一套 Playwright/Chromium，模块优先从 guizang 复制） |
...
- 本地 commit 取值：目标为 git 仓库时直接读 `.git` HEAD；复制式依赖（无 `.git`）**不做版本追踪**，本地版本视为未知，跳过自动更新（不记录任何基线文件，避免产生盘外落盘文件）。
- **本地版本未知（复制式依赖，无 `.git` 且无基线记录）且上游可达** → 跳过自动更新（**不写任何状态文件**）；如需刷新该类依赖，删除其目标目录后重跑 `ensure_deps.sh` 即重新拉取上游最新，避免对可能含本机定制的副本做盲目覆盖。
- 不一致（过期）→ **自动快进到最新（仅 git 类依赖）**：目标带 `.git` 时 `git fetch --depth 1 + reset --hard origin/<branch>`（保留 `node_modules` 等未跟踪文件）；独立仓库但缺 `.git` 时新鲜克隆上游并移植 `.git` 后 `reset --hard` 对齐（未跟踪文件保留）。monorepo 复制类依赖（dbs 系列 / xhs-real-keywords / no-ai-slop）**不做自动更新**：其一无 `.git` 无法比对；其二禁用版本基线写入以避免盘外落盘文件。如需刷新，删除目标目录后重跑脚本。
- 无法连接上游 → 跳过更新仅告警，**不阻塞**主流程（以本地现有版本继续）。
- 无来源的 `multi-search-engine` 无法自动校验最新性，由用户自行维护。

### 3. 超时与重试约束（防止网络调用无限挂起）

- 所有网络调用均被 `ensure_deps.sh` 包裹在超时内（GNU `timeout`，不可用时降级为「后台进程 + 超时 kill」兜底），**超时即中止该单次调用并继续，不会让整个脚本卡死**：
...
- 每次超时向 stderr 打印 `⏱️ [<label>] 超时(<秒>s)：<原因>` 与 `🔧 修复：<具体方案>`；诊断信息走 stderr，不污染标准输出数据通道。
- 常量集中在脚本顶部（`GIT_TIMEOUT` / `CLONE_TIMEOUT` / `FETCH_TIMEOUT` / `NPM_TIMEOUT` / `PLAYWRIGHT_TIMEOUT` / `MAX_RETRY`），调参只改一处。

### 4. 状态记录（已取消）

脚本**不再写任何状态文件**（无 `deps.json`、无 `.dep_commits`），以避免在本仓库产生盘外落盘文件。所有环境检测结果仅在运行时打印到 stdout/stderr 供即时查看，不做持久化。主 skill 判定依赖可用性仍依据运行时实际检测结果（存在性 + 深度依赖 `ok`），不依赖任何落盘记录。

---

## 环境检测实施细则

- **时机**：每次任务开始前，先于流程 11 步；禁止跳过。
- **依赖 skill 检测**：运行 `<SKILL_REPO_DIR>/ensure_deps.sh`（机制见上节）；退出码非 0 即中止。
- **主机运行时检测**（依赖检测通过后执行）：
...
- 各 dbs 子 skill 内部可能带有 `/dbs*` 路由导航（指向 dbs 家族其他 skill）。这些导航仅作原始形态保留——**本流程已由主 skill 按 11 步接管全部路由**，子 skill 内部的 `/dbs*` 指引是否执行由主 skill 决定，不得令流程脱离主编排。
- guizang 默认 task folder 为 `local-tests/<slug>/`；主 skill 在步骤 10 **必须显式传入** `<落盘根目录>/output/<最终标题_YYYYMMDD>` 作为 task folder，使产物落入已隔离的输出目录，避免仓库内临时残留。
...
- 禁止：硬编码落盘目录、未经询问即落盘、把中间产物（渲染脚本副本 / html / temp）留在落盘目录、把产物（`output/`）提交或推送到仓库（已列入 `.gitignore`）。
